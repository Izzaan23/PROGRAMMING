...
jarak_teks = st.sidebar.slider("Jarak Teks dari Garisan (Offset)", 0.5, 10.0, 2.5)
    ...
    if 'E' in df.columns and 'N' in df.columns:
        ...
        for i in range(len(df)):
            ...
            if papar_brg_dist:
                ...
                # Kira Vektor Normal (Serenjang) untuk tolak teks ke tepi secara center
                dx, dy = p2[0] - p1[0], p2[1] - p1[1]
                mag = np.sqrt(dx**2 + dy**2)
                nx, ny = -dy/mag, dx/mag # Arah serenjang
                
                # Pastikan teks ditolak ke LUAR poligon
                if np.dot([nx, ny], [mid_x - cx_m, mid_y - cy_m]) < 0:
                    nx, ny = -nx, -ny

                # Rotasi teks supaya selari garisan
                txt_rot = 90 - brg_deg
                if txt_rot < -90: txt_rot += 180
                if txt_rot > 90: txt_rot -= 180
                
                # PAPAR BEARING (Center ha/va)
                ax.text(mid_x + nx*jarak_teks, mid_y + ny*jarak_teks, brg_str, 
                        color='cyan' if papar_satelit else 'darkred', 
                        fontsize=saiz_font, fontweight='bold', ha='center', va='center', rotation=txt_rot)
                
                # PAPAR JARAK (Center ha/va di sebelah bertentangan)
                ax.text(mid_x - nx*jarak_teks, mid_y - ny*jarak_teks, f"{dst_val:.3f}m", 
                        color='white' if papar_satelit else 'blue', 
                        fontsize=saiz_font, fontweight='bold', ha='center', va='center', rotation=txt_rot)
        ...
        if papar_stn:
            for _, row in df.iterrows():
                ...
                ax.text(row['E']+(dx/mag)*stn_offset, row['N']+(dy/mag)*stn_offset, 
                        str(int(row['STN'])), color='yellow' if papar_satelit else 'black', 
                        fontweight='bold', ha='center', va='center', fontsize=saiz_font+2)

        if papar_satelit:
            try: cx.add_basemap(ax, crs="EPSG:4390", source="https://mt1.google.com/vt/lyrs=s&x={x}&y={y}&z={z}", zoom=18)
            except: pass
        ...
        with col_t1:
            ...
            st.download_button("🚀 Eksport GIS", data=json.dumps(geojson), file_name="gis.json")
        with col_t2:
            st.subheader("ℹ️ Maklumat Poligon")
            st.success(f"**Luas:** {kira_luas(df['E'].values, df['N'].values):.3f} m²")
            st.info("Klik pada mana-mana baris jadual untuk melihat info stesen.")
